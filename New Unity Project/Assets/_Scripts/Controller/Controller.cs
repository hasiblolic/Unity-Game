...
public class Controller : MonoBehaviour
{
	...
	public void SetWeapons(BaseItem rh, BaseItem lh)
	{
	...
	public void HandleAnimations(float vertical, float horizontal, float moveAmount)
	{
		if (isGrounded)
		{
			if (lockOn)
			{
				...
				anim.SetFloat("Vertical", f); //.2f, delta);
				...
				anim.SetFloat("Horizontal", s);// .2f, delta);
			}
			else
			{
				...
				anim.SetFloat("Vertical", f, .02f, Time.deltaTime);
				anim.SetFloat("Horizontal", 0);// 0.2f, delta);
			}
		}
		else
		{
	...
	public void PlayTargetItemAction(AttackInputs attackInput)
	{
		...
		ItemActionContainer iac = GetItemActionContainer(attackInput, currentActions);
	...
	public void LoadWeapon(BaseItem item, bool isLeft)
	{
	...
	public Transform FindLocakbleTarget()
	{
		...
		for (int i = 0; i < cols.Length; i++)
		{
			...
			if (lockable != null)
			{
				return lockable.GetLockOnTarget(mTransform);
			}
		}
